# Heroku ver. 4のローカルをHerokuで動かすために編集
...
def main(atk):
    ...
    for id in id_list[:9]: # 3番目から　[3:]
        ...
        # Herokuサーバーのダウンロード先directoryに変更
...
def downFile(id):
    ...
    # Chrome ドライバをインストールし、保存先のディレクトリに変更
...
# ここでは、シート1の現在価格と即決価格のいづれも数値がない＝売り切れ品を検知しリスト化（SKU列と共に）する。ダウンロードしたての出品中リストとMergeし、キャンセルリストにConcat する
# つまりAtkのDFは 列だけ残しておく. 現在価格、即決価格、SKU、アイテムIDをクエリすればいいはず
def cancel(atk, path, id, to_cancel, to_cancel2):
    ...
    # Dbeaver のAtklist4 テーブルからDFを
    df2 = atk['現在価格','即決価格','SKU','アイテムID']
    # df2 = df2.
    for row in df2:
...
def cancelUp(f, id):  # CSV整形とAuc へのアップロード
    # ココも同じく、Heroku上でCSV読み取りができると良いが
    app = xw.App(visible=False)
    wb = app.books.open(f)  # XWは、CSVを修正出来る  # (f)は一旦なし
    sheet = wb.sheets(1)
